...
class AddStudentForm(forms.Form):
    enrollment_number = forms.CharField(label="Enrollment Number",max_length=12,widget=forms.TextInput(attrs={"class": "form-control"}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["course_id"] = forms.ChoiceField(
            label="Course",
            choices=[(course.id, course.course_name) for course in Courses.objects.all()],
            widget=forms.Select(attrs={"class": "form-control"})
        )

        # Dynamically load session years
